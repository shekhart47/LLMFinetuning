...
```
    # LoRA configuration - ENHANCED for better generalization
    self.LORA_RANK = 16  # Increased from 8 for better capacity
    self.LORA_ALPHA = 32  # Increased proportionally
    self.LORA_DROPOUT = 0.05  # Reduced dropout for medical domain
    ...
    # Training hyperparameters - OPTIMIZED FOR GENERALIZATION
    self.NUM_EPOCHS = 2  # Reduced to prevent overfitting
    self.BATCH_SIZE = 2  # Per device batch size
    self.GRADIENT_ACCUMULATION_STEPS = 8  # Effective batch size = 32
    self.LEARNING_RATE = 1e-4  # Lower LR for better generalization
    self.WARMUP_RATIO = 0.05  # Shorter warmup
    self.WEIGHT_DECAY = 0.1  # Increased regularization
    ...
    self.EVAL_STEPS = 250  # More frequent evaluation
    self.SAVE_STEPS = 250
    ...
    self.DATALOADER_NUM_WORKERS = 8  # Increase for multiple GPUs
    self.GROUP_BY_LENGTH = True
    self.DDP_FIND_UNUSED_PARAMETERS = False  # Fixed parameter name
...
# For distributed training, use different device_map strategy
if world_size > 1:
    # Let DDP handle device placement
    device_map = None
    torch_dtype = config.BNB_4BIT_COMPUTE_DTYPE
else:
    # Single GPU - use auto device map
    device_map = "auto"
    torch_dtype = config.BNB_4BIT_COMPUTE_DTYPE
...
# For distributed training, move model to local device
if world_size > 1:
