...
class MedicalEvaluationConfig:
    def __init__(self):
        ...
        self.DATASET_DIR = "../../datasets/final_query_set/final_dataset/"
        
        # Medical NER model paths (LOCAL) - Updated to use accessible model
        self.MEDICAL_NER_MODEL_PATH = "./models/medical_ner"  # Download d4data/biomedical-ner-all
        self.SENTENCE_TRANSFORMER_PATH = "./models/sentence_transformer"  # Download pritamdeka/S-BioBert-snli-multinli-stsb
    ...
    def setup_medical_ner(self):
        ...
        try:
            if os.path.exists(self.config.MEDICAL_NER_MODEL_PATH):
                ...
                print("Please download: 'd4data/biomedical-ner-all' to the specified folder")
                print("This model supports: Drug, Disease, Chemical, Mutation, Species, CellLine, CellType")
                self.medical_ner = None
        except Exception as e:
            print(f"Error loading medical NER model: {e}")
            print("Falling back to regex-only entity extraction")
            self.medical_ner = None
    ...
    def extract_ner_entities(self, text: str) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            for entity in ner_results:
                ...
                # Map NER labels to our categories (updated for d4data/biomedical-ner-all)
                if any(label in entity_label for label in ['drug', 'chemical']):
                    entities['drugs'].add(entity_text)
                elif any(label in entity_label for label in ['disease', 'disorder']):
                    entities['symptoms'].add(entity_text)  # Diseases often manifest as symptoms
                elif 'mutation' in entity_label:
                    entities['conditions'].add(entity_text)
                elif any(label in entity_label for label in ['species', 'cellline', 'celltype']):
                    entities['procedures'].add(entity_text)  # Often related to diagnostic procedures
                else:
                    # Generic medical entity
                    entities['conditions'].add(entity_text)
