...
```
    ...
    # Training hyperparameters - OPTIMIZED FOR 2 H100s
    self.NUM_EPOCHS = 3
    self.BATCH_SIZE = 2  # Per device batch size
    self.GRADIENT_ACCUMULATION_STEPS = 8  # Effective batch size = 2*2*8 = 32
    ...
    # Multi-GPU configuration for 2 H100s
    self.BATCH_SIZE = 2  # Reduce per-device batch size for multi-GPU
    self.GRADIENT_ACCUMULATION_STEPS = 8  # Increase to maintain effective batch size
    
    # Distributed training settings
    self.DATALOADER_NUM_WORKERS = 8  # Increase for multiple GPUs
    self.DDPO_ENABLED = True  # Enable distributed training
